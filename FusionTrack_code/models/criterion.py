...
class ClipCriterion:
    ...
    def process_single_frame(self, model_outputs: dict, tracked_instances: List[TrackInstances], frame_idx: int, get_true_id=False):
        """
        ...
        for b in range(len(tracked_instances)):
            ...
            pass
        if get_true_id:
            return tracked_instances, new_trackinstances, unmatched_detections, true_id
    ...
    def get_loss_label(self, outputs, gt_trackinstances: List[TrackInstances], idx_to_gts_idx, get_true_id = False):
        """
        ...
        for b in range(len(pred_logits)):
            gt_ids[b][idx_to_gts_idx[b][0][idx_to_gts_idx[b][1] >= 0]] \
                = gt_trackinstances[b].ids[idx_to_gts_idx[b][1][idx_to_gts_idx[b][1] >= 0]]
        ...
        # if get_true_id:
        #     max_score = torch.max(pred_logits.sigmoid(), dim=1).indices
        #     true_id = (max_score == gt_labels)
        #     return loss, gt_labels[true_id]
        return loss
