...
class RuntimeTracker:
    ...
    def update(self, model_outputs: dict, tracks: List[TrackInstances]):
        ...
        if len(new_tracks) > 0:
            ...
            new_tracks.output_embed = new_tracks.output_embed[keep_indices_nms]
            new_tracks.query_embed = new_tracks.query_embed[keep_indices_nms]
            new_tracks.disappear_time = new_tracks.disappear_time[keep_indices_nms]
            new_tracks.labels = new_tracks.labels[keep_indices_nms]
        ...
        if len(tracks[0]) > 0 and len(new_tracks) > 0:
            ...
            if not keep_indices.all():
                ...
                new_tracks.scores = new_tracks.scores[keep_indices]
                new_tracks.output_embed = new_tracks.output_embed[keep_indices]
                new_tracks.query_embed = new_tracks.query_embed[keep_indices]
