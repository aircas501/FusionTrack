...
from .view_embedding import build_view_embedding
...
class FrozenBatchNorm2d(nn.Module):
    ...
    def forward(self, x):
        ...
        rm = self.running_mean.reshape(1, -1, 1, 1).to(x.device)
        eps = self.eps
...
class BackboneWithPEVE(nn.Module):
    """
    Backbone with Position Embedding and View Embedding.
    ...
    """
    def __init__(self, backbone: nn.Module, position_embedding: nn.Module, view_embedding: nn.Module = None):
        super(BackboneWithPEVE, self).__init__()
    ...
    def forward(self, ntensor: NestedTensor, view_ids: torch.Tensor = None) -> (List[NestedTensor], List[torch.Tensor]):
        """
        Args:
            ntensor: NestedTensor containing images
            view_ids: (B,) tensor of view IDs, optional
...
def build(config: dict) -> BackboneWithPEVE:
    ...
    return BackboneWithPEVE(backbone=backbone, position_embedding=position_embedding, view_embedding=view_embedding)
