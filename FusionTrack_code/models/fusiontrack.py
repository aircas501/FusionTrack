...
from .REID_model import build as build_reid_model
from .backbone import build as build_backbone_with_pe
from .REID_model import build_transformer
...
class FusionTrack(nn.Module):
    def __init__(self, backbone: BackboneWithPE, transformer: DeformableTransformer,
                 ...
                 reid: bool = False):
        super(FusionTrack, self).__init__()
    ...
    def forward(self, frame: NestedTensor, tracks: list[TrackInstances], obj_ids=None):
        if self.visualize:
        ...
        #         res["reid_cls_score"] = cls_score
        #         res["reid_global_feat"] = global_feat
        #     else:
        #         global_feat = self.query_reid(query_embed)
        #         res["reid_global_feat"] = global_feat
        return res
    ...
    def process_query_embedding_record(self, tracks: List[TrackInstances] | None):
        """
        Process query embedding record.
        """
        for t in tracks:
            t.query_embed_record.append(t.query_embed.detach().clone())
            if len(t.query_embed_record) > self.config.MODEL.MEMOTR.QUERY_RECORD_LEN:
                t.query_embed_record.pop(0)
